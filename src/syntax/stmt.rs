...
impl Parser {
    ...
    pub fn parse_stmt(&mut self) -> StatementType {
        match self.current_token.get_type() {
            TokenType::OpenCurly => {
                self.consume();
                StatementType::Block(Box::new(self.parse_block_stmt()))
            }
            TokenType::Return => {
                self.consume();
                let stmt: StatementType = StatementType::Return(Box::new(self.parse_expr(0)));
                self.expect(true, TokenType::Semicolon);
                stmt
            }
            _ => {
                let stmt: StatementType = StatementType::Expr(Box::new(self.parse_expr(0)));
                self.expect(true, TokenType::Semicolon);
                stmt
            }
        }
    ...
    pub fn parse_top_stmt(&mut self) -> StatementType {
        match self.current_token.get_type() {
            TokenType::Func => {
                StatementType::Decleration(Box::new(self.parse_decleration()))
            }
            _ => {
